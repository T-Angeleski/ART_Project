...
export class ART {
     ...
     */
    protected static compliment = false;
    /**
    * Number of inputs = 4. x,y coordinates + complement 
    */
    protected static num_inputs: number;
    /**
    * Number of nodes currently used. 
    */
    protected nodes_used: number;
    /**
    * Current number of maximum nodes.
    */
    protected static max_nodes: number;
    /**
     * Learning rate Beta, 1 for fast learning.
     */
    protected static beta = 0.8;
    /**
     * The Vigilance parameter. 
     */
    protected static vigilance = 0.8;
    /**
     * Winning category for the last input.
     */
    protected win_cat = -1;
    /**
     * Array for activation of the category nodes. Size max_nodes. 
     */
    protected static cat: number[];
    /**
     * Nodes eligibility array. Size max_nodes.
     */
    protected static elig: number[];
    /**
     * Commitment of cat node. Size max_nodes.
     */
    protected static commit: boolean[];
    /**
     * Weights matrix[max_nodes][ART.num_inputs], input -> cat. 
     */
    protected static weight: number[][];
    /**
     * Sum of weights for each node. Size max_nodes.
     */
    protected static sum_weights: number[];
    /**
     * Sum of intersected weights and inputs, i.e. Min(weights,inputs) for FuzzyArt.
     */
    protected static sum_IW: number[];
    /**
     * Number of maximum nodes.
     */
    protected static yes: boolean;
 ...
 */
    constructor(max_nodes: number, vigilance: number, beta: number,
        compliment: boolean, MAX: number) {
        ART.num_inputs = 4;
        ART.MAX = MAX;
        ART.beta = beta;
    ...
    /**
         ...
         */
    public ActivateArt(SignalX: number, SignalY: number, d: TDimension): void {
        /**
         * The inputs.
         */
        if (ART.compliment)
            ...
            while (!match) {
                ...
                if (ART.compliment) {
                    ...
                    for (let i = 0; i < ART.num_inputs / 2; ++i)
                        if (input[i] != -1) ++sum_inputs;
                } else {
                    sum_inputs = 0;
                    for (let i = 0; i < ART.num_inputs; ++i)
                        if (input[i] != -1) sum_inputs += input[i];
                }//DEBUG
                ...
                if ((ART.sum_IW[winner] / sum_inputs) >= ART.vigilance) {
                    ...
                    for (let i = 0; i < this.nodes_used; ++i) {
                        if (i == winner) ART.cat[i] = 1;
                        else {
                            ART.cat[i] /= ART.cat[winner];
                            if (ART.cat[i] > 1) ART.cat[i] = 1;
                            if (ART.cat[i] < 0) ART.cat[i] = 0;
                        }
                    }
                    this.win_cat = winner;
     ...
     */
    private TrainArt(input: number[]): number {
     ...
     * @return
     */
    public XforNode(i: number, d: TDimension): number {
     ...
     * @return
     */
    public YforNode(i: number, d: TDimension):number {
		let y = (ART.weight[i][1] * d.height);
        return y;
    }
    public setVigilance(vigilance: number): void {
        ART.vigilance = vigilance;
    }
    public setBeta(beta: number): void {
        ART.beta = beta;
    }
    public setStyle(compliment: boolean): void {
        ART.compliment = compliment;
    }
    public setNumNodes(nodes: number): void {
        ART.max_nodes = nodes;
    }
    public getWinner() {
        return this.win_cat;
    }
    public initArt(vigilance: number,beta: number,compliment: boolean,nodes: number): void {
    ...
    public getRadius() {
        ...
        for (let y = 0.01; y < 1.0; y += 0.01)
        {
            ...
            for (let j = 0; j < 2; ++j)
            if (input[j] != -1.0) sum += Math.min(xy[j], input[j]);
